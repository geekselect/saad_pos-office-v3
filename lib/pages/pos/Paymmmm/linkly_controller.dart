...
class LinklyDataController extends GetxController {
  ...
  String merchant = '00';
  String txnType = 'P';
  int amtPurchase = 20000; // Amount in cents, e.g., $200.00
  String currencyCode = 'AUD';
  String txnRef = 'POS-REF-123'; // Unique reference number generated by your POS system
  bool cutReceipt = true; // Indicate whether to cut the receipt
  bool receiptAutoPrint = true; // Indicate where to print the receipt
  String sku = 'SKU006005520'; // Basket ID
  String opr = 'OPR00766|test'; // Operator reference (POS operator name and ID)
  String amt = 'AMT00520000'; // Total amount of the entire sale in cents
  String uid = 'UID0367f619126-5fa7-4351-8585-1d4f813019d2'; // Unique identifier for the sale
  String nme = 'TestPOS'; // Name of the POS
  String ver = '4.1.00'; // Version of the POS
  String vnd = 'VND036907b3267-c37d-49c1-a8a2-429f937748c6'; // POS vendor ID
  String pcm = '1'; // POS Capabilities Matrix (first byte = POS can scan Barcode)


  Future<void> sendPurchaseRequest(dynamic id) async {
    // SharedPreferences prefs = await SharedPreferences.getInstance();
    // String appToken = prefs.getString(Constants.headerToken.toString()) ?? '';
    // var completer = Completer<void>();
    ...
    // print(newAmount);
    var url = Uri.parse(
        'https://rest.pos.sandbox.cloud.pceftpos.com/v1/sessions/$id/transaction?async=false');
    var headers = {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ${linklyDataModel.value.data!.token.toString()}',
      'Accept': 'application/json',
